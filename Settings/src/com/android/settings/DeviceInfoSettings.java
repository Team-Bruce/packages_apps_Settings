...
public class DeviceInfoSettings extends SettingsPreferenceFragment {
    ...
    private static final String KEY_BRUCE_VERSION = "bruce_version";
	...
	setValueSummary(KEY_BRUCE_VERSION, "ro.bruce.version");
        findPreference(KEY_BRUCE_VERSION).setEnabled(true);
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        } else if (preference.getKey().equals(KEY_BRUCE_VERSION)) {
	    System.arraycopy(mHits, 1, mHits, 0, mHits.length-1);
	    mHits[mHits.length-1] = SystemClock.uptimeMillis();
            if (mHits[0] >= (SystemClock.uptimeMillis()-500)) {
                Intent intent = new Intent(Intent.ACTION_MAIN);
                intent.setClassName("com.android.settings",
                        com.android.settings.bruce.BruceLogoActivity.class.getName());
                try {
                    startActivity(intent);
                } catch (Exception e) {
                    Log.e(LOG_TAG, "Unable to start activity " + intent.toString());
